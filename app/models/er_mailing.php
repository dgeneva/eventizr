...
class ER_Mailing extends CI_Model{
	...
	function findNewsletter($token){
		$data = array();
		$query = $this->db->get_where('er_emailing', array('token' => $token)); 
	  	if ($query->num_rows() > 0){
...
/*
	function createEntry($email){
		...
		$this->db->insert('er_mailing', $data);
	...
	private function _distribute ($id, $type){
		
		echo 'bulk_send';
	}
	
	function distribute ($id, $type){
		$this->_distribute('1', 'newsletter');
